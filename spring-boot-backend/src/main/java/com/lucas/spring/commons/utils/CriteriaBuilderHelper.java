...
public class CriteriaBuilderHelper {
  ...
  public <T> Page<T> getPagedResult(
    ...
    final CriteriaQuery<T> sortedCriteriaQuery =
            CriteriaBuilderHelper.applySortable(criteriaQuery, cb, root, pageable);

    // Creates a query
    final TypedQuery<T> query = entityManager.createQuery(sortedCriteriaQuery);

    // Creates a pageable query
    final TypedQuery<T> pageableQuery = CriteriaBuilderHelper.applyPageable(query, pageable);

    return PageableExecutionUtils.getPage(
            pageableQuery.getResultList(),
            pageable, () -> CriteriaBuilderHelper.getQueryMaxResult(query));
  }
}
