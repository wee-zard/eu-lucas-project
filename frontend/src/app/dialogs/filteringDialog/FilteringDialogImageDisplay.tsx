...
const FilteringDialogImageDisplay = () => {
  ...
  useEffect(() => {
    if (
      ...
      getImageByFilters(request, filteringPageableProperties).then(
        (pageableResponse) => {
          if (pageableResponse) {
  ...
  const handleRenderOfEmptyBody = () => {
    return !response ? (
      <StyledEmptyBody>
        Szűrési feltétel még nem kerül megadásra!
      </StyledEmptyBody>
    ) : response?.pageItems.length === 0 ? (
      <StyledEmptyBody>
        A megadott szűrési feltétel üres eredményt adott vissza!
      </StyledEmptyBody>
    ) : null;
  };
  ...
  const isElementPresentInSelectedImages = (imageEntity: ImageDto) =>
    (selectedImage &&
      !!selectedImage.images.find((image) => image.id === imageEntity.id)) ??
    false;
            ...
            {response?.pageItems.map((imageEntity, index) => (
              <StyledCard
                ...
                onClick={(event) => {
                  ...
                  handleClickOnGlobalRippleEffect(
                    event,
                    IdUtils.GetFilteredImageCardDivId(index)
                  );
                }}
              >
                <CardMedia
...
const StyledFilterImageHolder = styled(StyledScrollBarHolder)<{}>((props) => ({
  padding: "8px",
  marginRight: "2px",
}));
